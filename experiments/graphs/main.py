...
def exp0b() :
    ...
    opa_csv_path = f"../bare-metal/results/{exp}/opa"
    owsm_csv_path = f"../bare-metal/results/{exp}/owsm"

    print(f"|- Considering experiment {exp}")

    #data = pd.DataFrame(columns=["use_case", "num_requests", "mean_opa", "variance_opa", "mean_owsm", "variance_owsm"])
    ...
    for file in os.listdir(opa_csv_path):
        ...
        data = pd.DataFrame({"number_request": range(1, len(owsm_data) + 1), "OWSM": owsm_data["ttime"], "OPA": opa_data["ttime"]})
        ...
        plt.ylabel('Elapsed time (µs)')  # Replace with relevant column name
        ...
        #result = mannwhitneyu(data['OWSM'].tolist(), data['OPA'].tolist())
        #stat, p_value = wilcoxon(data['OWSM'].tolist(), data['OPA'].tolist())
...
def exp1() :
    ...
    for use_case in range(1,4):    
        ...
        # use_case_data["log_num_requests"] = [math.log(val) for val in use_case_data["num_requests"]]

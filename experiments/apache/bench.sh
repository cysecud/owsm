...
for r in "${numRequests[@]}"; do
   owsm_result=$(ab -n $r -c $c -p $INPUT_NAME -T application/json $owsm_URL)
   owsm_avg_time_ms=$(tprExtract "$owsm_result")

   opa_result=$(ab -n $r -c $c -p $INPUT_NAME -T application/json $opa_URL)
   opa_avg_time_ms=$(tprExtract "$opa_result")

   echo "$r,$owsm_avg_time_ms,$opa_avg_time_ms" >> $outputFile
done

echo "Complete benchmark for $INPUT_NAME"
